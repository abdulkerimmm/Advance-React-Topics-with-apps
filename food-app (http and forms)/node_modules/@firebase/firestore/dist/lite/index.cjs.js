 ...
 */ function __PRIVATE_queryOrderBy(t) {
 ...
 */ function __PRIVATE_queryToTarget(t) {
    const e = __PRIVATE_debugCast(t);
    if (!e.B) if ("F" /* LimitType.First */ === e.limitType) e.B = __PRIVATE_newTarget(e.path, e.collectionGroup, __PRIVATE_queryOrderBy(e), e.filters, e.limit, e.startAt, e.endAt); else {
        // Flip the orderBy directions since we want the last results
        const t = [];
        for (const r of __PRIVATE_queryOrderBy(e)) {
            const e = "desc" /* Direction.DESCENDING */ === r.dir ? "asc" /* Direction.ASCENDING */ : "desc" /* Direction.DESCENDING */;
            t.push(new OrderBy(r.field, e));
        }
        ...
        e.B = __PRIVATE_newTarget(e.path, e.collectionGroup, t, e.filters, e.limit, r, n);
    }
    return e.B;
}
 ...
 */ class __PRIVATE_NumericIncrementTransformOperation extends TransformOperation {
    constructor(t, e) {
        super(), this.serializer = t, this.$ = e;
    }
}
...
class __PRIVATE_DatastoreImpl extends class Datastore {} {
    ...
    X() {
        if (this.Z) throw new FirestoreError(w, "The client has already been terminated.");
    }
    /** Invokes the provided RPC with auth and AppCheck tokens. */    R(t, e, r) {
        return this.X(), Promise.all([ this.authCredentials.getToken(), this.appCheckCredentials.getToken() ]).then((([n, i]) => this.connection.R(t, e, r, n, i))).catch((t => {
    ...
    /** Invokes the provided RPC with streamed results with auth and AppCheck tokens. */    g(t, e, r, n) {
        return this.X(), Promise.all([ this.authCredentials.getToken(), this.appCheckCredentials.getToken() ]).then((([i, s]) => this.connection.g(t, e, r, i, s, n))).catch((t => {
            throw "FirebaseError" === t.name ? (t.code === p && (this.authCredentials.invalidateToken(), 
            this.appCheckCredentials.invalidateToken()), t) : new FirestoreError(A, t.toString());
        }));
    }
...
async function __PRIVATE_invokeRunAggregationQueryRpc(t, e, r) {
    var n;
    const i = __PRIVATE_debugCast(t), {request: s, tt: o} = function __PRIVATE_toRunAggregationQueryRequest(t, e, r) {
    ...
    }(i.serializer, __PRIVATE_queryToTarget(e), r), a = s.parent;
...
/** A "context" object passed around while parsing user data. */ class __PRIVATE_ParseContextImpl {
    ...
    lt(t) {
        return __PRIVATE_createError(t, this.settings.methodName, this.settings.ct || !1, this.path, this.settings.ht);
    }
    ...
    ot(t) {
        ...
        if (__PRIVATE_isWrite(this.rt) && U.test(t)) throw this.lt('Document fields cannot begin and end with "__"');
    }
}
...
class __PRIVATE_DeleteFieldValueImpl extends FieldValue {
    _toFieldTransform(t) {
        if (2 /* UserDataSource.MergeSet */ !== t.rt) throw 1 /* UserDataSource.Update */ === t.rt ? t.lt(`${this._methodName}() can only appear at the top level of your update data`) : t.lt(`${this._methodName}() cannot be used with set() unless you pass {merge:true}`);
 ...
 */
    return function __PRIVATE_parseSentinelFieldValue(t, e) {
        ...
        if (!__PRIVATE_isWrite(e.rt)) throw e.lt(`${t._methodName}() can only be used with update() and set()`);
 ...
 */ (t, e), null;
    ...
    e.path && e.fieldMask.push(e.path), t instanceof Array) {
        ...
        if (e.settings.st && 4 /* UserDataSource.ArrayArgument */ !== e.rt) throw e.lt("Nested arrays are not supported");
    ...
    return function __PRIVATE_parseScalarValue(t, e) {
        ...
        if (t instanceof DocumentReference) {
            ...
            if (!n.isEqual(r)) throw e.lt(`Document reference is for database ${n.projectId}/${n.database} but should be for database ${r.projectId}/${r.database}`);
            return {
                referenceValue: __PRIVATE_toResourceName(t.firestore._databaseId || e.databaseId, t._key.path)
            };
        }
        throw e.lt(`Unsupported field value: ${__PRIVATE_valueDescription(t)}`);
    }
    /**
...
function __PRIVATE_validatePlainObject(t, e, r) {
    if (!__PRIVATE_looksLikeJsonObject(r) || !function __PRIVATE_isPlainObject(t) {
        ...
        throw "an object" === n ? e.lt(t + " a custom object") : e.lt(t + " " + n);
    }
}
...
function query(t, e, ...r) {
    ...
    e instanceof AppliableConstraint && n.push(e), n = n.concat(r), function __PRIVATE_validateQueryConstraintArray(t) {
        ...
        if (e > 1 || e > 0 && r > 0) throw new FirestoreError(T, "InvalidQuery. When using composite filters, you cannot use more than one filter at the top level. Consider nesting the multiple filters within an `and(...